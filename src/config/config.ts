...
export const config = {
  ...
  rateLimit: process.env['NODE_ENV'] === 'production'
    ? { windowMs: 15 * 60 * 1000, max: 100 } // production limit
    : { windowMs: 15 * 60 * 1000, max: 1000000 }, // effectively disabled in dev
  ...
  gps: {
    updateInterval: parseInt(process.env['GPS_UPDATE_INTERVAL'] || '30', 10), // seconds
    accuracyThreshold: parseInt(process.env['GPS_ACCURACY_THRESHOLD'] || '50', 10), // meters
  },
